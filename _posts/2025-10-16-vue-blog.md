...
3) router.js 파일 생성하고 코드 추가 / 이해 안해도 됨
...
//props 도 저렇게 넘겨주기 가능 / router-view 태그 사이에 주석 넣으면 안나옴
```

- router-view 태그

이 컴포넌트도 되고 저 컴포넌트도 되는 태그
...
//:작명 하면 경로 뒤에 아무 문자나 입력하면 component 보여줘 라는 뜻
```


```javascript
$route.params.id
//여기서 id 는 내가 router.js 에서 작명한 그거
//내가 url 뒤에 쓰는 문자를 나타냄
```


0 1 2 등등 변하는 수 url 에 넣기


```javascript
<div>
    <div v-for="content in 블로그글" :key="content">
        <router-link :to="'/detail/' + content.number">
            <h5>{% raw %}{{{% endraw %} content.title {% raw %}}}{% endraw %}</h5>
        </router-link >
        <p>{% raw %}{{{% endraw %} content.content {% raw %}}}{% endraw %}</p>
        <p>{% raw %}{{{% endraw %} content.date {% raw %}}}{% endraw %}</p>
    </div>
</div>

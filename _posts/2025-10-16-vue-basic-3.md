...
//li 반복문 돌릴 배열을 수정
      ...
      <span :class="{ done: todo.done }">{% raw %}{{{% endraw %} todo.text {% raw %}}}{% endraw %}</span>
      <button @click="removeTodo(todo)">X</button>
    </li>
  </ul>
  <button @click="hideCompleted = !hideCompleted">
    {% raw %}{{{% endraw %} hideCompleted ? 'Show all' : 'Hide completed' {% raw %}}}{% endraw %}
  </button>
</template>
...
<template>
  <p>할 일 id: {% raw %}{{{% endraw %} todoId {% raw %}}}{% endraw %}</p>
  <button @click="todoId++">다음 할 일 가져오기</button> //클릭하면 id 증가
  <p v-if="!todoData">로딩...</p>
  <pre v-else>{% raw %}{{{% endraw %} todoData {% raw %}}}{% endraw %}</pre>
</template>
```
